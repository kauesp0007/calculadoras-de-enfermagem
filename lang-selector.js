 ...
 * Também mantém a área de conta, a moderação administrativa e controles
...
function langSelectorInit() {
  ...
  if (current) {
    if (langFlag) langFlag.src = current.dataset.flag;
    if (langText) langText.textContent = current.textContent.trim();
  }
}
...
const ACCOUNT_EXTRA_I18N = {
  ...
  it: {"MINHA CONTA":"IL MIO ACCOUNT","Início":"Home","Idioma":"Lingua","País":"Paese","Método de login":"Metodo di accesso","Data de cadastro":"Data di registrazione","Último acesso":"Ultimo accesso","Editar nome":"Modifica nome","Conta":"Account","Premium (Júnior)":"Premium (Junior)","Digite um nome válido (mínimo 2 caracteres).":"Inserisci un nome valido (almeno 2 caratteri).","Nome atualizado com sucesso!":"Nome aggiornato con successo!","Tema":"Tema","Receber novidades":"Ricevi aggiornamenti","Padrão":"Predefinito","Grande":"Grande","Muito grande":"Molto grande","Extra grande":"Extra grande","Máximo":"Massimo","Mais recentes":"Più recenti","Mais antigos":"Più vecchi","Ordem alfabética":"Ordine alfabetico","Visitas":"Visite","Páginas únicas":"Pagine uniche","Tempo total":"Tempo totale","Admin":"Admin","Usuário":"Utente","Calculadora":"Calcolatore","Escala":"Scala","Artigo":"Articolo","Protocolo":"Protocollo","Download":"Download","Curso":"Corso","Simulado":"Simulazione","Biblioteca":"Biblioteca","Página":"Pagina","Inglês":"Inglese","Espanhol":"Spagnolo","Alemão":"Tedesco","Italiano":"Italiano","Francês":"Francese","Hindi":"Hindi","Chinês":"Cinese","Árabe":"Arabo","Japonês":"Giapponese","Russo":"Russo","Coreano":"Coreano","Turco":"Turco","Holandês":"Olandese","Polonês":"Polacco","Sueco":"Svedese","Indonésio":"Indonesiano","Vietnamita":"Vietnamita","Ucraniano":"Ucraino"},
...
function currentAccountLanguage() {
  try {
    if (window.AccountI18n && window.AccountI18n.getLanguage) {
      const value = String(window.AccountI18n.getLanguage()).toLowerCase();
...
function loadAccountMenuLocalizer() {
  if (window.AccountMenuI18n) { window.AccountMenuI18n.init(); return; }
  if (document.getElementById("account-menu-localizer-loader")) return;
  const script = document.createElement("script");
  script.id = "account-menu-localizer-loader";
  script.src = "/js/conta-menu-localizer.js";
  script.async = true;
  script.onload = function () { if (window.AccountMenuI18n) window.AccountMenuI18n.init(); };
  script.onerror = function () { console.warn("Não foi possível carregar o localizador do menu de conta."); };
...
function loadAccountExtraLocalizer() {
  if (window.AccountExtraLocalizer) { window.AccountExtraLocalizer.localize(); return; }
  if (document.getElementById("account-extra-localizer-loader")) return;
  const script = document.createElement("script");
  script.id = "account-extra-localizer-loader";
  script.src = "/js/account-extra-localizer.js";
  script.async = true;
  script.onload = function () { if (window.AccountExtraLocalizer) window.AccountExtraLocalizer.localize(); };
  script.onerror = function () { console.warn("Não foi possível carregar o localizador complementar da conta."); };
  document.head.appendChild(script);
}
...
function accountLanguageSync() {
  ...
  if (window.MutationObserver && document.body) {
    ...
    window.__accountLanguageObserver = new MutationObserver(function (mutations) {
      const dict = ACCOUNT_EXTRA_I18N[currentAccountLanguage()];
      if (!dict) return;
