 ...
 *
 * API centralizada de autorização. O plano efetivo é calculado a partir do
 * perfil do usuário, incluindo expiração e licença vitalícia.
 */
(function (window) {
  "use strict";
  ...
  function _premiumStillValid(p) {
    ...
    var expires = p.planExpiresAt instanceof Date
      ? p.planExpiresAt
      : (p.planExpiresAt.toDate ? p.planExpiresAt.toDate() : new Date(p.planExpiresAt));
    return !Number.isNaN(expires.getTime()) && expires.getTime() > Date.now();
  }

  function getRole() {
    var p = _profile();
    return p && p.role ? p.role : (_user() ? "user" : "guest");
  }

  function getPlan() {
    var p = _profile();
    if (!p || !_premiumStillValid(p)) return "free";
    return p.plan || "free";
  }
  ...
  if (window.Auth && window.Auth.onProfileChange) {
    window.Auth.onProfileChange(function () { _refresh(); });
  }
