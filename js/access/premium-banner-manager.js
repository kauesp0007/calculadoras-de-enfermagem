...
(function (window, document) {
    ...
    function isAccountPage() { return (window.location.pathname || "").indexOf("/conta/") === 0; }
    function isPremium() {
        try {
    ...
    function loadAdSenseForEligibleUser() {
        if (isAccountPage() || isPremium() || window.__adsenseLoaded) return;
        try {
            var consent = localStorage.getItem("cookieConsent");
            if (consent === "refused" || (consent === "managed" && localStorage.getItem("ad_storage") === "denied")) return;
        } catch (_) {}
    ...
    function _getRoot() {
        ...
        if (!_root) { _root = document.createElement("div"); _root.id = "premium-banner-root"; document.body.insertBefore(_root, document.body.firstChild); }
        return _root;
    }
    function mount(opts) {
        opts = opts || {};
    ...
    function unmount() { if (_root) _root.innerHTML = ""; _mounted = false; }
    function isMounted() { return _mounted; }
    function showGuestFallback() {
        if (isAccountPage() || isPremium() || dismissed() || document.getElementById("premium-welcome-banner-root") || document.getElementById("premium-guest-fallback") || !document.body) return;
        ...
        root.innerHTML = '<style>#premium-guest-fallback{position:fixed;top:18px;right:18px;z-index:2147483000;width:min(380px,calc(100vw - 36px));font-family:inherit}.pgf-card{position:relative;background:#fff;border:1px solid #d9e5f3;border-radius:16px;box-shadow:0 14px 38px rgba(20,53,89,.18);padding:18px;color:#1f2937}.pgf-card h2{margin:0 34px 8px 0;color:#1A3E74;font-size:17px;line-height:1.3}.pgf-card p{margin:0 0 12px;font-size:14px;line-height:1.5}.pgf-row{display:flex;align-items:center;justify-content:space-between;gap:12px}.pgf-price{font-weight:700;color:#1A3E74}.pgf-button{display:inline-flex;align-items:center;border:0;border-radius:10px;background:#1A3E74;color:#fff;padding:10px 13px;font-size:13px;font-weight:700;text-decoration:none}.pgf-close{position:absolute;top:8px;right:8px;width:32px;height:32px;border:0;background:transparent;color:#64748b;cursor:pointer;display:grid;place-items:center}</style><div class="pgf-card"><button type="button" class="pgf-close" aria-label="'+t[4]+'" title="'+t[4]+'"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 384 512" width="15" height="20" aria-hidden="true" fill="currentColor"><path d="M342.6 182.6 236.3 288l106.3 105.4c9.4 9.4 9.4 24.6 0 34s-24.6 9.4-34 0L202.3 322 96 427.4c-9.4 9.4-24.6 9.4-34 0s-9.4-24.6 0-34L168.3 288 62 182.6c-9.4-9.4-9.4-24.6 0-34s24.6-9.4 34 0L202.3 254l106.3-105.4c9.4-9.4 24.6-9.4 34 0s9.4 24.6 0 34z"/></svg></button><h2>'+t[0]+'</h2><p>'+t[1]+'</p><div class="pgf-row"><span class="pgf-price">'+t[2]+'</span><a class="pgf-button" href="/conta/assinatura.html?lang='+encodeURIComponent(lang)+'">'+t[3]+'</a></div></div>';
        document.body.appendChild(root);
        root.querySelector(".pgf-close").addEventListener("click", function () { try { localStorage.setItem(DISMISS_KEY, "1"); } catch (_) {} root.remove(); });
    }
    function initGuestExperience() {
        if (isAccountPage()) return;
        loadAdSenseForEligibleUser();
        setTimeout(function () { loadAdSenseForEligibleUser(); showGuestFallback(); }, 1800);
    }
    window.AccessModules.bannerManager = { mount: mount, unmount: unmount, isMounted: isMounted };
    if (document.readyState === "loading") document.addEventListener("DOMContentLoaded", initGuestExperience, { once: true });
    else initGuestExperience();
