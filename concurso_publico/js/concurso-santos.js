(() => {
  ...
  function loadAnalytics() {
    try {
      ...
      return {
        candidate: raw.candidate || { name: '' },
        time: raw.time || { totalMs: 0, accesses: 0 },
        opened: raw.opened || {},
        features: raw.features || {},
        materials: raw.materials || {},
        external: Array.isArray(raw.external) ? raw.external : [],
        simulados: raw.simulados || {}
      };
    } catch { return { candidate: { name: '' }, time: { totalMs: 0, accesses: 0 }, opened: {}, features: {}, materials: {}, external: [], simulados: {} }; }
  }
  function persistAnalytics() { try { localStorage.setItem(ANALYTICS_KEY, JSON.stringify(analytics)); } catch (e) { } }
  function trackFeature(key) { analytics.features[key] = (analytics.features[key] || 0) + 1; persistAnalytics(); }
  function trackOpened(id) {
    const e = analytics.opened[id] || { count: 0, firstAt: null, lastAt: null };
    e.count++; e.lastAt = new Date().toISOString(); if (!e.firstAt) e.firstAt = e.lastAt;
    analytics.opened[id] = e;
    trackFeature('card:abrir');
  }
  function trackMaterial(id, kind) {
    const m = analytics.materials[id] || {};
    if (kind === 'baixado') m.downloadedAt = new Date().toISOString();
    if (!m.accessedAt) m.accessedAt = new Date().toISOString();
    analytics.materials[id] = m;
    trackFeature('material:' + kind);
    renderMaterials();
  }
  function addExternalReading(text) {
    const t = String(text || '').trim(); if (!t) return false;
    analytics.external.push({ text: t, at: new Date().toISOString() });
    persistAnalytics();
    renderMaterials();
    return true;
  }
  function saveCandidateName(name) {
    analytics.candidate.name = String(name || '').trim();
    trackFeature('candidato:identificar');
    renderCandidate();
  }
  ...
  function flushSessionTime() {
    const now = Date.now();
    analytics.time.totalMs += Math.max(0, now - sessionStart);
    sessionStart = now;
    persistAnalytics();
  }
  let analytics = loadAnalytics();
  let sessionStart = Date.now();
  analytics.time.accesses = (analytics.time.accesses || 0) + 1;
  persistAnalytics();
  ...
  function cycleStatus(id) {
    ...
    setTopicState(id, { status: next }); trackFeature('status'); toast(`${id}: ${STATUS_LABEL[next]}`);
  }
  ...
  function renderGuideAps(g) {
    ...
    const parameters = g.parameters ? `<div class="aps-section"><h3>${esc(g.parameters.title)}</h3><p class="aps-lead">${esc(g.parameters.text)}</p><div class="aps-dir-list">${g.parameters.items.map(p => `<div class="aps-dir"><b>${esc(p.t)}</b><span>${esc(p.d)}</span></div>`).join('')}</div></div>` : '';
    const recognizedTeams = g.recognizedTeams ? `<div class="aps-section"><h3>${esc(g.recognizedTeams.title)}</h3><div class="aps-dir-list">${g.recognizedTeams.items.map(t => `<div class="aps-dir"><b>${esc(t.t)}</b><span>${esc(t.d)}</span></div>`).join('')}</div></div>` : '';
    const keywords = g.keywords.map(k => `<span class="keyword">${esc(k)}</span>`).join('');
  ...
  function bindTabsScroll() {
    const nav = $('.study-tabs');
    const left = $('#tabsScrollLeft');
    const right = $('#tabsScrollRight');
    if (!nav || !left || !right) return;
    const amount = () => Math.max(160, Math.round(nav.clientWidth * 0.6));
    left.addEventListener('click', () => { nav.scrollBy({ left: -amount(), behavior: 'smooth' }); });
    right.addEventListener('click', () => { nav.scrollBy({ left: amount(), behavior: 'smooth' }); });
  }
  ...
  function bindFilters() {
    ...
    $('#areaSelect').addEventListener('change', e => { filters.area = e.target.value; filters.highYield = false; trackFeature('filter'); renderFilters(); });
  ...
  function bindNotes() {
    ...
    $('#copyNotes').addEventListener('click', async () => { trackFeature('notes'); try { await navigator.clipboard.writeText(ta.value); toast('Notas copiadas'); } catch { ta.select(); document.execCommand('copy'); toast('Notas copiadas'); } });
  }
  ...
  function bindTimer() {
    $('#timerStart').addEventListener('click', () => { if (timerId) return; trackFeature('pomodoro'); timerId = setInterval(() => { timerSeconds--; timerRender(); if (timerSeconds <= 0) { clearInterval(timerId); timerId = null; timerSeconds = 25 * 60; timerRender(); toast('Sessão concluída — registre o que aprendeu e seus erros.'); } }, 1000) });
  ...
  // ===== RELÓGIO + TEMPO DE DEDICAÇÃO =====
  let sessionClockTimer = null;
  function pad2(n) { return String(n).padStart(2, '0'); }
  function renderClock() {
    const el = $('#studyClockTime');
    if (el) {
      const s = Math.floor((Date.now() - sessionStart) / 1000);
      el.textContent = `${pad2(Math.floor(s / 3600))}:${pad2(Math.floor((s % 3600) / 60))}:${pad2(s % 60)}`;
    }
    const ded = $('#dedicationTotal');
    if (ded) ded.textContent = fmtDuration(analytics.time.totalMs + (Date.now() - sessionStart));
  }
  function initClock() {
    renderClock();
    sessionClockTimer = setInterval(renderClock, 1000);
    window.addEventListener('pagehide', flushSessionTime);
    document.addEventListener('visibilitychange', () => { if (document.hidden) flushSessionTime(); });
  }

  // ===== RENDER: CANDIDATO / ESTATÍSTICAS / MATERIAIS =====
  function renderCandidate() {
    const hello = $('#candidateHello');
    const input = $('#candidateName');
    if (hello) hello.textContent = analytics.candidate.name ? `Olá, ${analytics.candidate.name}! Seus dados estão salvos neste navegador.` : '';
    if (input && analytics.candidate.name) input.value = analytics.candidate.name;
  }

  function renderStats() {
    const grid = $('#statsGrid'); if (!grid) return;
    const totalTopics = data.topics.length;
    const openedCount = Object.keys(analytics.opened).length;
    const openedPct = totalTopics ? Math.round(openedCount / totalTopics * 100) : 0;
    const toolsUsed = TOOLS.filter(t => (analytics.features[t[0]] || 0) > 0).length;
    const toolsPct = TOOLS.length ? Math.round(toolsUsed / TOOLS.length * 100) : 0;
    const matTotal = MATERIAIS.length;
    const matAccessed = MATERIAIS.filter(m => analytics.materials[m.id]).length;
    const matPct = matTotal ? Math.round(matAccessed / matTotal * 100) : 0;
    const totalMs = analytics.time.totalMs + (Date.now() - sessionStart);
    const accesses = analytics.time.accesses || 1;
    const totalHours = (totalMs / 3600000).toFixed(1);
    const avgPerAccess = fmtDuration(Math.max(0, totalMs / accesses));
    const sims = Object.values(analytics.simulados || {});
    const simDone = sims.length;
    const simCorrect = sims.reduce((a, s) => a + (s.correct || 0), 0);
    const simWrong = sims.reduce((a, s) => a + (s.wrong || 0), 0);
    const extCount = analytics.external.length;
    const bar = (pct, color) => `<div class="stat-bar"><span style="width:${Math.min(100, Math.max(0, pct))}%;background:${color || 'var(--navy)'}"></span></div>`;

    grid.innerHTML = `
      <article class="stat-card">
        <div class="stat-head">${icon('target', 'icon')}<h4>Aproveitamento no site</h4></div>
        <div class="stat-donut-wrap"><div class="donut" style="--p:${toolsPct}"><div class="donut-center"><b>${toolsPct}%</b><small>aproveitamento</small></div></div></div>
        <p class="stat-sub">${toolsUsed} de ${TOOLS.length} ferramentas utilizadas pelo candidato.</p>
      </article>
      <article class="stat-card">
        <div class="stat-head">${icon('book', 'icon')}<h4>Cards abertos e lidos</h4></div>
        <b class="stat-num">${openedCount}<span> de ${totalTopics}</span></b>
        ${bar(openedPct)}
        <p class="stat-sub">${openedPct}% dos cards de estudo foram abertos pelo candidato.</p>
      </article>
      <article class="stat-card">
        <div class="stat-head">${icon('list', 'icon')}<h4>Mini-simulados</h4></div>
        <b class="stat-num">${simDone}<span> questionários</span></b>
        <div class="stat-duo"><span class="ok">${simCorrect} acertos</span><span class="bad">${simWrong} erros</span></div>
        <p class="stat-sub">${simDone === 0 ? 'Em breve: os simulados expressos serão contabilizados aqui.' : `${simDone} questionários realizados.`}</p>
      </article>
      <article class="stat-card">
        <div class="stat-head">${icon('clock', 'icon')}<h4>Tempo de dedicação</h4></div>
        <b class="stat-num">${totalHours}<span> horas no total</span></b>
        <div class="stat-duo"><span>${accesses} acesso(s)</span><span>média ${avgPerAccess}</span></div>
        <p class="stat-sub">Tempo acumulado do candidato dentro da página, somado a cada acesso.</p>
      </article>
      <article class="stat-card">
        <div class="stat-head">${icon('download', 'icon')}<h4>Materiais consultados</h4></div>
        <b class="stat-num">${matAccessed}<span> de ${matTotal}</span></b>
        ${bar(matPct, '#16a34a')}
        <p class="stat-sub">${matPct}% dos materiais didáticos foram acessados/baixados.</p>
      </article>
      <article class="stat-card">
        <div class="stat-head">${icon('note', 'icon')}<h4>Leituras fora do site</h4></div>
        <b class="stat-num">${extCount}<span> item(ns)</span></b>
        <p class="stat-sub">Conteúdos lidos pelo candidato fora do site, registrados manualmente.</p>
      </article>`;
  }
  ...
  function renderMaterials() {
    ...
    if (list) {
      const dl = MATERIAIS.map(m => {
        const rec = analytics.materials[m.id];
        const url = m.path + encodeURIComponent(m.file);
        const check = rec ? `<span class="mat-check" title="Acessado">✓<small>${fmtDataBr(rec.accessedAt)}</small></span>` : `<span class="mat-check empty">·</span>`;
        return `<div class="mat-item"><div class="mat-info"><b>${esc(m.label)}</b><span>${esc(m.desc)}</span></div><div class="mat-actions">${check}<a class="btn mat-download" href="${url}" download="${esc(m.file)}" data-mat-id="${m.id}" target="_blank" rel="noopener">Baixar</a></div></div>`;
      }).join('');
      const src = (data.sources || []).map((s, i) => {
        const rec = analytics.materials['src' + i];
        const check = rec ? `<span class="mat-check" title="Acessado">✓<small>${fmtDataBr(rec.accessedAt)}</small></span>` : `<span class="mat-check empty">·</span>`;
        return `<div class="mat-item"><div class="mat-info"><b>${esc(s.label)}</b><span>${esc(s.type || 'oficial')}</span></div><div class="mat-actions">${check}<a class="btn mat-open" href="${esc(s.url)}" target="_blank" rel="noopener noreferrer" data-mat-id="src${i}">Consultar</a></div></div>`;
      }).join('');
      list.innerHTML = `<h3 class="mat-h">Downloads (PDF)</h3><div class="mat-grid">${dl}</div><h3 class="mat-h">Links oficiais para consulta</h3><div class="mat-grid">${src}</div>`;
      $$('[data-mat-id]', list).forEach(a => a.addEventListener('click', () => trackMaterial(a.dataset.matId, a.classList.contains('mat-download') ? 'baixado' : 'acessado')));
    }
    const ext = $('#materialsExternal');
    if (ext) {
      const items = analytics.external.map((e, i) => `<li><span class="mat-check">✓<small>${fmtDataBr(e.at)}</small></span><span>${esc(e.text)}</span><button class="icon-btn" data-ext-del="${i}" aria-label="Remover item">${icon('x')}</button></li>`).join('');
      ext.innerHTML = `<h3 class="mat-h">O que o candidato leu fora do site</h3><div class="ext-form"><input type="text" id="externalInput" placeholder="Ex.: Lei 8.080/90 comentada, livro de fundamentos..."><button class="btn primary" id="externalSave" type="button">Salvar</button></div><ul class="ext-list">${items || '<li class="ext-empty">Nenhum item registrado ainda.</li>'}</ul>`;
      $('#externalSave').addEventListener('click', () => { if (addExternalReading($('#externalInput').value)) { $('#externalInput').value = ''; toast('Registro salvo'); } });
      $('#externalInput').addEventListener('keydown', e => { if (e.key === 'Enter') $('#externalSave').click(); });
      $$('[data-ext-del]', ext).forEach(b => b.addEventListener('click', () => { analytics.external.splice(Number(b.dataset.extDel), 1); persistAnalytics(); renderMaterials(); }));
    }
  }
...
<script>window.onload=function(){window.print()}<\/script></body></html>`;
  ...
  function bindActions() {
    ...
    $('#candidateSave').addEventListener('click', () => { saveCandidateName($('#candidateName').value); toast('Nome do candidato salvo'); });
    $('#candidateName').addEventListener('keydown', e => { if (e.key === 'Enter') $('#candidateSave').click(); });
