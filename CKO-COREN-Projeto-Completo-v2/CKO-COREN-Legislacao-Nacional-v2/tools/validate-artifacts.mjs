...
for (const set of SETS) {
  ...
  for (const item of items) {
    ...
    for (const e of validateAgainstSchema(item, schema, label)) {
      errs++;
      findings.push({ code: 'ARTV-001', severity: 'P0', subject: `${set.file}:${label}`,
...
for (const s of (await readJson('evidence/sources/evidence-sources.json')).sources) {
  if (!actIds.has(s.canonical_id)) {
    findings.push({ code: 'ARTV-005', severity: 'P1', subject: s.evidence_source_id,
      message: `evidence source órfã: canônico "${s.canonical_id}" não existe no corpus.` });
  }
}
...
console.log(JSON.stringify({ instances: total, result: report.result, findings: findings.length,
  detail: findings.slice(0, 10) }, null, 2));
