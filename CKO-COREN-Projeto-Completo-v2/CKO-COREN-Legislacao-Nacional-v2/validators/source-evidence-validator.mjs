...
const ACCEPTABLE_AUTHORITY = new Set([
  'OFFICIAL_REGIONAL', 'OFFICIAL_FEDERAL_SYSTEM_COPY', 'OFFICIAL_TRANSPARENCY_PORTAL',
]);
...
export function validateSourceEvidence(act, ctx = {}) {
  ...
  if (!ACCEPTABLE_AUTHORITY.has(src.authority_class)) {
    findings.push({ code: 'EVID-001', severity: 'P0', subject: act.canonical_id,
      message: `authority_class "${src.authority_class}" não é fonte oficial aceitável.` });
  }
  if (!/^https:\/\//.test(src.url || '')) {
    findings.push({ code: 'EVID-002', severity: 'P1', subject: act.canonical_id,
      message: 'URL da fonte não usa https.' });
  }

  const refs = act.evidence?.source_refs || [];
  if (!refs.length) {
    findings.push({ code: 'EVID-003', severity: 'P0', subject: act.canonical_id,
  ...
  for (const [i, d] of (act.devices || []).entries()) {
    ...
    if (!f) {
      findings.push({ code: 'EVID-008', severity: 'P0', subject: `${act.canonical_id}#device-${i}`,
      ...
      findings.push({ code: 'EVID-009', severity: 'P0', subject: `${act.canonical_id}#device-${i}`,
