...
function gerarPDFGlobal(options) {
    const {
        titulo = 'Informe de la Calculadora',
        subtitulo = 'Informe de Cálculo Asistencial',
        nomeArquivo = 'informe.pdf',
        seletorConteudo = '.main-content-wrapper'
    } = options;

    console.log(`Iniciando generación de PDF para: ${titulo}`);
    const elementoParaImprimir = document.querySelector(seletorConteudo);

    if (!elementoParaImprimir) {
        alert('Error: No fue posible encontrar el contenido principal para generar el PDF.');
        console.error(`Elemento con selector "${seletorConteudo}" no encontrado.`);
        return;
    }
    ...
    pdfHeader.innerHTML = `
        ...
        <p style="font-size: 10px; color: #999; margin-top: 10px;">Generado en: ${new Date().toLocaleString('es-ES')}</p>
    `;
