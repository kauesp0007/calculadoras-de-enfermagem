...
// =============================================
// MONITORAMENTO CORE WEB VITALS (VIA CDN)
// =============================================

// 1. Carrega a biblioteca oficial do Google dinamicamente (via CDN)
function carregarWebVitals() {
  return new Promise((resolve, reject) => {
    const script = document.createElement('script');
    script.src = 'https://cdn.jsdelivr.net/npm/web-vitals@4/dist/web-vitals.iife.js';
    script.onload = () => resolve();
    script.onerror = () => reject(new Error('Falha ao carregar Web Vitals'));
    document.head.appendChild(script);
  });
}

// 2. Função que será chamada quando as métricas forem medidas
function enviarMetricas(metrica) {
  // Exibe no console do navegador (F12) para você ver os números
  console.log(`🔵 [Web Vitals] ${metrica.name}: ${metrica.value} (${metrica.rating})`);
  
  // (OPCIONAL) Se você tiver Google Analytics 4, descomente as 3 linhas abaixo:
  // if (window.gtag) {
  //   window.gtag('event', metrica.name, { value: Math.round(metrica.value), metric_rating: metrica.rating });
  // }
}

// 3. Inicia a medição assim que o usuário interagir com a página
async function iniciarMonitoramento() {
  try {
    await carregarWebVitals();
    
    // Verifica se a biblioteca foi carregada corretamente
    if (window.webVitals) {
      const { onLCP, onINP, onCLS, onFCP, onTTFB } = window.webVitals;
      
      // Ativa a escuta das principais métricas (essenciais para o Google)
      onLCP(enviarMetricas);   // Largest Contentful Paint (carregamento)
      onINP(enviarMetricas);   // Interaction to Next Paint (responsividade)
      onCLS(enviarMetricas);   // Cumulative Layout Shift (estabilidade visual)
      
      // (Opcional) Métricas secundárias
      onFCP(enviarMetricas);   // First Contentful Paint
      onTTFB(enviarMetricas);  // Time to First Byte
      
      console.log('✅ Monitoramento Core Web Vitals ativado com sucesso!');
    } else {
      console.warn('⚠️ Biblioteca web-vitals não encontrada.');
    }
  } catch (erro) {
...
// Aguarda o DOM carregar para não bloquear o carregamento da página
