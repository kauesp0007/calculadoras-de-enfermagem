...
function determinarNivelResultado(resultadoElement) {
  if (!resultadoElement) return "";
  const niveisExistentes = ["nivel-1", "nivel-2", "nivel-3", "nivel-4"];
  const dataNivel = resultadoElement.dataset?.nivel || resultadoElement.dataset?.resultadoNivel;
  if (dataNivel) {
    const normalizado = String(dataNivel).trim().toLowerCase();
    if (/^nivel-\d$/.test(normalizado)) return normalizado;
    if (/^\d$/.test(normalizado)) return `nivel-${normalizado}`;
  }

  const classNames = Array.from(resultadoElement.classList).map(e => e.toLowerCase());
  const classText = classNames.join(" ");
  const nivelJaDefinido = niveisExistentes.find(e => classNames.includes(e));
  if (nivelJaDefinido) return nivelJaDefinido;

  const mapaClasses = {
    "bg-low-risk": "nivel-1",
    "bg-medium-risk": "nivel-2",
    "bg-high-risk": "nivel-4",
    "bg-very-high-risk": "nivel-4",
    "bg-warning-yellow-light": "nivel-2",
    "bg-danger-red-light": "nivel-4",
    "bg-danger-red": "nivel-4",
    "bg-success-green": "nivel-1",
    "bg-success-green-light": "nivel-1",
    "bg-info-alerta": "nivel-3",
    "bg-info-orange-light": "nivel-3",
    "bg-elpo-risco": "nivel-4",
    "bg-elpo-sem-risco": "nivel-1",
    "bg-sofa-low": "nivel-1",
    "bg-sofa-medium": "nivel-2",
    "bg-sofa-high": "nivel-4",
    "bg-qsofa-low": "nivel-1",
    "bg-qsofa-high": "nivel-4",
    "bg-apache-low": "nivel-1",
    "bg-apache-medium": "nivel-2",
    "bg-apache-high": "nivel-4",
    "bg-saps-low": "nivel-1",
    "bg-saps-medium": "nivel-2",
    "bg-saps-high": "nivel-4",
    "bg-pelod-baixo": "nivel-1",
    "bg-pelod-moderado": "nivel-2",
    "bg-pelod-alto": "nivel-3",
    "bg-pelod-muito-alto": "nivel-4",
    "bg-lanss-negativa": "nivel-1",
    "bg-lanss-positiva": "nivel-4",
    "bg-fast-negativo": "nivel-1",
    "bg-fast-positivo": "nivel-4",
    "bg-cincinnati-negativo": "nivel-1",
    "bg-cincinnati-positivo": "nivel-4",
    "bg-downes-leve": "nivel-2",
    "bg-downes-moderado": "nivel-3",
    "bg-downes-grave": "nivel-4",
    "bg-aldrete-alta": "nivel-1",
    "bg-aldrete-observacao": "nivel-2",
    "bg-aldrete-instavel": "nivel-4"
  };

  const mapeado = classNames.find(e => mapaClasses[e]);
  if (mapeado) return mapaClasses[mapeado];

  const nivel4Keywords = [
    "very-high",
    "muito-alto",
    "high-risk",
    "grave",
  ...
  if (nivel4Keywords.some(e => classText.includes(e))) return "nivel-4";
  if (nivel3Keywords.some(e => classText.includes(e))) return "nivel-3";
  if (nivel2Keywords.some(e => classText.includes(e))) return "nivel-2";
  if (nivel1Keywords.some(e => classText.includes(e))) return "nivel-1";
  return "";
}

function aplicarNivelResultado(elementoResultado) {
  if (!elementoResultado) return;
  const nivelAtual = determinarNivelResultado(elementoResultado);
  if (!nivelAtual) return;
  const niveis = ["nivel-1", "nivel-2", "nivel-3", "nivel-4"];
  const jaAplicado = elementoResultado.classList.contains(nivelAtual)
    && niveis.every(nivel => nivel === nivelAtual || !elementoResultado.classList.contains(nivel));
  if (jaAplicado) return;
  elementoResultado.classList.remove(...niveis);
  elementoResultado.classList.add(nivelAtual);
}

function aplicarNiveisResultados(raiz = document) {
  raiz.querySelectorAll(".resultado").forEach(aplicarNivelResultado);
}
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  aplicarNiveisResultados();
  const observer = new MutationObserver(e => {
    e.forEach(e => {
      if (e.type === "attributes" && e.target?.classList?.contains("resultado")) {
        aplicarNivelResultado(e.target);
      }
      if (e.type === "childList") {
        e.addedNodes.forEach(e => {
          if (!(e instanceof Element)) return;
          if (e.classList.contains("resultado")) aplicarNivelResultado(e);
          e.querySelectorAll?.(".resultado").forEach(aplicarNivelResultado);
        });
      }
    });
  });
