...
def processar_arquivos():
    ...
    for idioma in idiomas:
        ...
        for filename in os.listdir(pasta_alvo):
            ...
            # Injeta a classe underline e hover:text-[#153260] nos links das referências bibliográficas
            ...
            if re.search(padrao_link_ref, conteudo_modificado, re.DOTALL):
                # Verifica se as classes já não foram adicionadas
                def injetar_classes_ref(match):
                    classes_atuais = match.group(1)
                    if 'underline' not in classes_atuais:
                         # Adiciona underline e hover:text-[#153260] (se text-[#1A3E74] existir, mantemos)
            ...
            if re.search(padrao_link_duvidas, conteudo_modificado):
                 def injetar_classes_duvidas(match):
                     ...
                     if 'underline' not in classes_atuais:
                          # Adiciona underline (se já não tiver)
                          ...
                          # Certifica-se de ter w-fit (opcional, mas recomendado)
            ...
            if alterou_algo:
                ...
                # print(f"  {C_VERDE}Corrigido:{RESET} {filename}") # Descomente para ver cada arquivo
            else:
