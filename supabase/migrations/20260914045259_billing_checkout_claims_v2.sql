...
declare
  ...
  v_expires timestamptz := v_now + make_interval(secs => greatest(60, least(p_lease_seconds, 7200)));
begin
  if p_provider not in ('stripe','asaas') or nullif(trim(p_user_id),'') is null then raise exception 'invalid_checkout_claim'; end if;
  insert into public.billing_checkout_claims(provider,user_id,status,created_at,expires_at,completed_at,last_error)
  values (p_provider,trim(p_user_id),'active',v_now,v_expires,null,null)
  on conflict (provider,user_id) do update
    set status='active',created_at=v_now,expires_at=v_expires,completed_at=null,last_error=null
  where public.billing_checkout_claims.status in ('failed','completed') or public.billing_checkout_claims.expires_at <= v_now;
  return found;
end;
$$;
...
begin
  update public.billing_checkout_claims
     set status='failed',completed_at=now(),expires_at=now(),last_error=left(coalesce(p_error,''),2000)
   where provider=p_provider and user_id=trim(p_user_id) and status='active';
  return found;
end;
$$;
